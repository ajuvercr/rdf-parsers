...
mod format_tests {
    use crate::{
        parse,
        trig::{Lang, Rule, SyntaxKind},
    };

    fn format_trig(input: &str, width: usize) -> String {
    ...
    #[test]
    fn format_triples_inside_graph() {
        // Too wide for one line: break after ';', indenting the second clause.
        let input = r#"GRAPH <b> { <a> <b> <c> }"#;
        let out = format_trig(input, 64);
        assert_eq!(out, "GRAPH <b> { <a> <b> <c> }\n");
    }

    #[test]
    fn format_triples_inside_graph_long() {
        let input = r#"GRAPH <b> { <a> <b> <c>. <a> <b> <c> }"#;
        let out = format_trig(input, 16);
        assert_eq!(out, "GRAPH <b> {\n  <a> <b> <c>.\n\n  <a> <b> <c>\n\n}\n");
    }

    #[test]
    fn format_triples_inside_graph_bn() {
        let input = r#"GRAPH <b> { [ <a> <b> ; <a> <b> ] a foaf:Friend }"#;
        let out = format_trig(input, 16);
        assert_eq!(
            out,
            "GRAPH <b> {\n  [\n    <a> <b>; <a> <b>\n  ] a foaf:Friend\n}\n"
