...
fn process_node<'a>(
    ...
    Box::pin(async move {
        let map: HashMap<&str, &JsonLdVal> = members.iter().map(|(k, _, _, v)| (k.as_str(), v)).collect();
            ...
            if let JsonLdVal::Object(rev_members, _) = rev_val {
                for (prop, _, val_span, val) in rev_members {
                    ...
                    if let Some(pred_iri) = expand_iri(active, prop, true, false) {
                        ...
                        let objects =
                            collect_objects(state, active, loader, val, val_span, graph, None).await;
                        for obj in objects {
        ...
        for (key, key_span, val_span, val) in members {
            ...
            let objects = collect_objects(state, active, loader, val, val_span, graph, term_def).await;
            if objects.is_empty() {
                continue;
            }
            let s = span.clone();
                ...
                Triple {
                    ...
                    po: vec![Spanned(
                        PO {
                            ...
                            object: objects.into_iter().map(|o| Spanned(o, val_span.clone())).collect(),
                        },
                        key_span.clone(),
                    )],
...
mod tests {
    ...
    fn test_predicate_span_points_to_compact_iri_key_not_expanded_iri() {
        ...
        let triples = triples_of(&t);
        assert_eq!(triples.len(), 1);
        assert_eq!(predicate_iri(&triples[0]), Some("http://xmlns.com/foaf/0.1/name"));
    ...
    fn test_spans_are_independent_per_property() {
        ...
        let pred_starts: Vec<usize> = triples.iter()
            .map(|t| t.po[0].predicate.1.start)
            .collect();
        ...
        let obj_starts: Vec<usize> = triples.iter()
            .map(|t| t.po[0].object[0].1.start)
            .collect();
